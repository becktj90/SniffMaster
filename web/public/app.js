...
const SPACE_COAST_DAYBOOK = {
    "01-31": [{
        ...
        detail: "Ham the chimp rode a suborbital test from Cape Canaveral, clearing a path for America's first crewed Mercury flight."
    }, ],
    "02-20": [{
        year: 1962,
        title: "Friendship 7",
        detail: "John Glenn launched from Cape Canaveral and became the first American to orbit Earth."
    }, ],
    "04-05": [{
            year: 1973,
            title: "Pioneer 11",
            detail: "Pioneer 11 departed Cape Canaveral on the mission that would later visit Jupiter and become the first spacecraft to reach Saturn."
        },
        {
            year: 1991,
            title: "STS-37 Atlantis",
            detail: "Atlantis lifted off from KSC carrying the Compton Gamma Ray Observatory, opening a major new era in space astrophysics."
        },
        {
            year: 2010,
            title: "STS-131 Discovery",
            detail: "Discovery launched from KSC on a station resupply mission packed with racks, cargo, and the Leonardo MPLM."
        },
    ],
    "05-05": [{
        year: 1961,
        title: "Freedom 7",
        detail: "Alan Shepard launched from Cape Canaveral and became the first American in space."
    }, ],
    "07-16": [{
        year: 1969,
        title: "Apollo 11",
        detail: "Saturn V thundered off Pad 39A at KSC on the first crewed lunar landing mission."
    }, ],
    "08-12": [{
        year: 1960,
        title: "Echo 1A",
        detail: "Cape Canaveral launched the giant metallized balloon that became one of the earliest communications-satellite experiments."
    }, ],
    "09-05": [{
        year: 1977,
        title: "Voyager 1",
        detail: "Voyager 1 left Cape Canaveral and began the grand tour that would rewrite the map of the outer solar system."
    }, ],
    "11-12": [{
        year: 1981,
        title: "STS-2 Columbia",
        detail: "NASA launched the first reusable crewed spacecraft flight with the same orbiter, Columbia, from KSC."
    }, ],
    "12-18": [{
        year: 1999,
        title: "STS-103 Discovery",
        detail: "Discovery launched from KSC on the Hubble Space Telescope servicing mission that restored the observatory's science momentum."
    }, ],
};
...
async function loadMelodyBank() {
    if (!melodyBankPromise) {
        melodyBankPromise = fetch("/melody_library.h", {
                cache: "no-store"
            })
            .then((res) => {
...
async function playMelodyFromSnapshot() {
    ...
    await playMelodyByTitle(normalizedTitle, {
        source: "snapshot"
    });
}
...
function paranormalSignal(d) {
    ...
    return {
        ...
        note: ts ?
            `${d.paranormalEntity || "Unknown"} was last seen ${fmtAge(ts)}. The radar is visualizing cached scan intensity from the live environmental signal stack.` : "A cached anomaly result exists, but its timing could not be reconstructed cleanly. The display is still based on gas and room-context signals.",
    };
}
...
function chemistryItems(d) {
    return [{
            name: "PM2.5",
            value: num(d.outdoorPm25, NaN),
            unit: "ug/m3",
            threshold: 35
        },
        {
            name: "PM10",
            value: num(d.outdoorPm10, NaN),
            unit: "ug/m3",
            threshold: 50
        },
        {
            name: "O3",
            value: num(d.outdoorO3, NaN),
            unit: "ug/m3",
            threshold: 100
        },
        {
            name: "NO2",
            value: num(d.outdoorNo2, NaN),
            unit: "ug/m3",
            threshold: 40
        },
        {
            name: "CO",
            value: num(d.outdoorCo, NaN),
            unit: "ug/m3",
            threshold: 4000
        },
        {
            name: "SO2",
            value: num(d.outdoorSo2, NaN),
            unit: "ug/m3",
            threshold: 20
        },
        {
            name: "NH3",
            value: num(d.outdoorNh3, NaN),
            unit: "ug/m3",
            threshold: 50
        },
...
function broBadgeInfo(d) {
    ...
    if (vsc >= 70 || bioPeak >= 55 || score >= 70) return {
        text: "Needs work",
        tone: "danger"
    };
    if (bioPeak >= 30 || score >= 45) return {
        text: "Worth watching",
        tone: "warn"
    };
    return {
...
function drawHeroScope(current, history) {
    ...
    if (latest.voc >= 3.0) {
        alertItems.push({
            tone: "danger",
            text: `VOC heavy at ${latest.voc.toFixed(2)} ppm.`
        });
    } else if (latest.voc >= 1.5) {
        alertItems.push({
            tone: "warn",
            text: `VOC elevated at ${latest.voc.toFixed(2)} ppm.`
        });
    }
    ...
    if (latest.dVoc >= 1.0 || vocTrendNow >= 0.45) {
        alertItems.push({
            tone: "danger",
            text: `Fresh gas spike: ${fmtSigned(latest.dVoc, 2)} dVOC.`
        });
    } else if (latest.dVoc >= 0.3 || vocTrendNow >= 0.2) {
        alertItems.push({
            tone: "warn",
            text: `New gas activity rising faster than baseline.`
        });
    }
    ...
    if (latest.airScore >= 60) {
        alertItems.push({
            tone: "danger",
            text: `Room load is poor: ${Math.round(latest.airScore)}/100.`
        });
    } else if (latest.airScore >= 40 || cleanTrendNow <= -6) {
        alertItems.push({
            tone: "warn",
            text: `Cleanliness is slipping: ${Math.round(latestClean)}% clean.`
        });
    }
    ...
    while (alertItems.length < 3) {
        if (!alertItems.length) {
            alertItems.push({
                tone: "good",
                text: usingHistory ? "Trace is within normal operating bands." : "Waiting for enough real samples to flag anomalies."
            });
        } else if (alertItems.length === 1) {
            alertItems.push({
                tone: "neutral",
                text: `Room index ${Math.round(latest.airScore)}/100. Lower room score is better.`
            });
        } else {
            alertItems.push({
                tone: "neutral",
                text: `Latest rate: VOC ${fmtSigned(vocTrendNow, 2)} · Clean ${fmtSigned(cleanTrendNow, 0)}%.`
            });
        }
    }
...
async function fetchHistory() {
    try {
        const res = await fetch("/api/history?count=1008", {
            cache: "no-store"
        });
        if (!res.ok) return;
...
async function fetchSniffHistory() {
    try {
        const res = await fetch("/api/sniff-history?count=16", {
            cache: "no-store"
        });
        if (!res.ok) return;
...
const trajectoryArcade = (() => {
    ...
    function drawParticles() {
        runtime.particles.forEach((particle) => {
            const {
                x,
                y
            } = worldToScreen(particle.x, particle.y);
    ...
    function drawHud(p) {
        ...
        const subsystems = [{
            ...
            {
                name: "PROPELLANT",
                pct: runtime.propMass,
                thresh: 25
            },
            {
                name: "GUIDANCE",
                pct: runtime.phase >= PHASE.TRANSIT ? 100 : 60,
                thresh: 0
            },
            {
                name: "WEAPONS",
                pct: runtime.gators.length ? (runtime.gatorsKilled / Math.max(1, runtime.gatorsTotal)) * 100 : 100,
                thresh: 0
            },
        ];
        let sy = 48;
...
$("melody-library-play-selected")?.addEventListener("click", async () => {
    ...
    await playMelodyByTitle(selected.title, {
        source: "library"
    });
    renderMelodyLibrary(lastData);
});
...
$("melody-library-play-device")?.addEventListener("click", async () => {
    ...
    if (!selected) return;
    setMelodyLibrarySelection(selected.title);
    await queueRemoteAction("play_melody", {
        melodyKey: selected.key
    });
    renderMelodyLibrary(lastData);
});
