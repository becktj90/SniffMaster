...
#include "bsec2.h"
#include "secrets.h"          // WiFi, Adafruit IO, OpenAI credentials
...
// Last melody played — shown in the SniffMaster report
...
static char   dcSmellRadar[300] = "";   // GPT smell radar narrative for V8
...
static void saveBsecState() {
  ...
  if (homeBase.ready && homeBaseLooksValid(homeBase)) {
    PersistedHomeBaseline stored = {
      ...
      HOME_BASE_VER,
      homeBase.calmSamples,
      static_cast<uint8_t>(homeBase.ready ? 1 : 0),
      {0, 0, 0},
      homeBase.voc,
      homeBase.iaq,
      homeBase.co2,
...
bool fetchGPTSassyMsg(int iaq, float voc, float co2, float tempF, float hum,
  ...
  if (!err) {
    ...
    // GPT smell radar narrative (for V8)
...
// Blynk support removed in this build.
...
static void renderBootStatusPage() {
  ...
  display.print(F("Presence: disabled"));
